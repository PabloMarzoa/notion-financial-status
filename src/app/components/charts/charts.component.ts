...
@Component({
  ...
  template: `
    <div class="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-2 gap-4 sm:gap-6">
      ...
      <div class="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-6 shadow-xl relative overflow-hidden backdrop-blur-md">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-sm sm:text-base 2xl:text-lg font-semibold text-slate-900 dark:text-white tracking-tight">Gastos por Categoría</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">Distribución de los gastos en el período</p>
          </div>
          <span class="text-xs font-mono px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg border border-slate-200 dark:border-slate-700 whitespace-nowrap">
        ...
        <div *ngIf="stats?.categoryBreakdown && stats!.categoryBreakdown.length > 0; else noCategories">
          ...
          <div class="h-3.5 sm:h-4 w-full bg-slate-100 dark:bg-slate-800 rounded-full flex overflow-hidden mb-5 sm:mb-6 shadow-inner">
            <div
          ...
          <div class="space-y-2.5 sm:space-y-3 max-h-72 sm:max-h-80 2xl:max-h-96 overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
            ...
            >
              ...
              <div class="text-right flex-shrink-0">
                ...
                <div class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">{{ cat.percentage }}%</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <ng-template #noCategories>
          <div class="py-12 text-center text-slate-400 dark:text-slate-500 text-xs sm:text-sm">
      ...
      <div class="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-6 shadow-xl relative overflow-hidden backdrop-blur-md">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-sm sm:text-base 2xl:text-lg font-semibold text-slate-900 dark:text-white tracking-tight">Evolución Mensual</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">Comparativa Ingresos vs Gastos</p>
          </div>
          <div class="flex items-center space-x-2 sm:space-x-3 text-[11px] sm:text-xs">
            <span class="flex items-center space-x-1">
              <span class="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-emerald-500 dark:bg-emerald-400"></span>
              <span class="text-slate-600 dark:text-slate-300">Ingresos</span>
            </span>
            <span class="flex items-center space-x-1">
              <span class="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-rose-500 dark:bg-rose-400"></span>
          ...
          <div class="space-y-3 sm:space-y-4 max-h-72 sm:max-h-80 2xl:max-h-96 overflow-y-auto pr-1 custom-scrollbar">
            <div
              *ngFor="let m of stats!.monthlyBreakdown"
              class="p-2.5 sm:p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800/80"
            >
              <div class="flex items-center justify-between mb-2">
                <span class="text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">{{ m.label }}</span>
                <span
                  class="text-[11px] sm:text-xs font-mono font-medium px-2 py-0.5 rounded-md whitespace-nowrap"
                  [ngClass]="m.balance >= 0 ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-500/20'"
                >
              ...
              <div class="space-y-1.5 text-xs">
                <div class="flex items-center space-x-2">
                  ...
                  <span class="w-16 sm:w-20 text-right font-mono text-emerald-600 dark:text-emerald-400 text-[10px] sm:text-[11px] flex-shrink-0">
                ...
                <div class="flex items-center space-x-2">
                  ...
                  <span class="w-16 sm:w-20 text-right font-mono text-rose-600 dark:text-rose-400 text-[10px] sm:text-[11px] flex-shrink-0">
